...
use storage_units::utils::serialize::{read_size, write_size, SIZE_SIZE};
...
impl fmt::Display for ParseStagingIdError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ParseStagingIdError::Base58Encoding(_) => write!(f, "Expected Base58 encoded string"),
            ParseStagingIdError::InvalidSize(found) => write!(
                f,
                "not of a valid size (expected {}, but got {})",
                SIZE_SIZE, found
            ),
        }
    }
...
mod test {
    ...
    fn test_encode_decode(id: StagingId) {
        ...
        assert!(
            id == decoded,
            "encode/decode failed for id: {:?} (encoded: {:?}, decoded: {:?})",
            id,
            encoded,
            decoded
        );
