...
impl Time {
    pub fn now() -> Self { Time(SystemTime::now()) }
}
impl Deref for Time {
...
impl fmt::Display for Time {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let s : String = format!("{}", ::humantime::format_rfc3339(self.0)).chars().take(10).collect();
        write!(f, "{}", s)
    }
}
impl From<SystemTime> for Time {
    fn from(st: SystemTime) -> Time { Time(st) }
}
    ...
    pub fn between(t1: Time, t2: Time) -> Self {
        match t1.duration_since(*t2) {
            ...
            Err(_) => match t2.duration_since(*t1) {
                Ok(duration) => Duration::from(duration),
                Err(err)     => unreachable!("error when trying to get duration between 2 dates, {}", err)
            }
        }
    }
}
